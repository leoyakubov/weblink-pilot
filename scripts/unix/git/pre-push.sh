...
if [ "$backend_style_status" = "PASS" ]; then
  ...
  if "$repo_root/scripts/unix/quality/backend-coverage.sh" 2>&1 | tee "$backend_coverage_log"; then
...
if [ "$frontend_tests_status" = "PASS" ]; then
  ...
  if "$repo_root/scripts/unix/quality/frontend-coverage.sh" 2>&1 | tee "$frontend_coverage_log"; then
