...
cleanup() { rm -f "$backend_tests_log" "$backend_coverage_log" "$frontend_tests_log" "$frontend_coverage_log" "$frontend_build_log"; }
trap cleanup EXIT
...
if [ "$backend_tests_status" != "PASS" ] || [ "$backend_coverage_status" != "PASS" ] || [ "$frontend_tests_status" != "PASS" ] || [ "$frontend_coverage_status" != "PASS" ] || [ "$frontend_build_status" != "PASS" ]; then
  exit 1
fi
